...
const agregarCliente = (e) => {
  ...
  sendAgregarCliente(formData);
};

const sendAgregarCliente = async (formData) => {
  const datos = new FormData();
  Object.keys(formData).forEach((key) => {
    datos.append(key, formData[key]);
  });

  const url = "/dashboard/clientes/agregar";
  try {
...
const editarCliente = (e) => {
  ...
  sendEditarCliente(formData);
};

const sendEditarCliente = async (formData) => {
  const datos = new FormData();
  Object.keys(formData).forEach((key) => {
    datos.append(key, formData[key]);
  });

  const url = "/dashboard/clientes/editar";
...
const eliminarCliente = async (e) => {
  ...
  const url = "/dashboard/clientes/eliminar";

  const resultado = await Swal.fire({
    title: "Estas Seguro?",
    text: "Se Eliminara este Cliente de Manera Permanente",
    icon: "warning",
...
export { agregarCliente, editarCliente, eliminarCliente };
